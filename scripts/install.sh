...
echo ""
echo "Next step — wire your MCP client(s) and the full recall/capture loop:"
echo ""
echo -e "  ${BOLD}perseus-vault install-client --hooks --rules${RESET}"
echo ""
echo "  Autodetects Claude Code / Codex / Cursor (or pass --client <name>;"
echo "  --all-detected wires every detected client), merges the MCP server"
echo "  registration into the client's config, points every client at ONE"
echo "  shared memory database, registers session lifecycle hooks (recall on"
echo "  SessionStart, hygiene on session end), and appends the memory usage"
echo "  rules to CLAUDE.md/AGENTS.md. Add --dry-run to preview every change."
echo ""
echo "Or configure any MCP host by hand:"
echo '  {'
echo '    "mcpServers": {'
echo '      "perseus-vault": {'
